...
from facts import EWASTE_FACTS
from prompts import ANALYSIS_PROMPT, LISTING_PROMPT
from utils import carbon_saving

load_dotenv()

st.set_page_config(
    page_title="EcoLens | E-Waste Value Detector",
    page_icon="♻️",
    layout="wide",
)
...
:root {
  ...
  --fg:          hsl(160,30%,12%);
  --card:        #FFFFFF;
  --primary:     hsl(153,70%,38%);
  --primary-dk:  hsl(153,70%,28%);
  --primary-lt:  hsl(153,60%,48%);
  --accent:      hsl(43,75%,50%);
  --border:      hsl(150,15%,85%);
  --secondary:   hsl(150,20%,92%);
  --subtle:      hsl(160,8%,55%);
  --faintest:    hsl(160,6%,72%);
  --shadow-card: 0 4px 24px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.03);
  --shadow-cta:  0 4px 20px rgba(15,138,95,0.25), 0 0 60px rgba(15,138,95,0.08);
...
/* Force Streamlit containers to use cream bg */
...
@keyframes pulseGlow { 0%,100%{box-shadow:0 0 15px rgba(15,138,95,.2)} 50%{box-shadow:0 0 40px rgba(15,138,95,.45)} }
...
.ebay-card:hover {
    border-color: rgba(15,138,95,0.35) !important;
    background: rgba(15,138,95,0.04) !important;
    transform: translateX(4px);
    box-shadow: 0 4px 20px rgba(15,138,95,0.08);
}

/* ── Background Orbs ─────────────────────────────────────── */
.orb { position:fixed; border-radius:50%; filter:blur(80px); pointer-events:none; z-index:0; }
.orb-1 { width:700px;height:700px; background:radial-gradient(circle,rgba(15,138,95,.06) 0%,transparent 70%); top:-128px;right:-128px; animation:blob 14s ease-in-out infinite; }
.orb-2 { width:500px;height:500px; background:radial-gradient(circle,rgba(201,162,39,.04) 0%,transparent 70%); top:33%;left:-80px; animation:blob 18s ease-in-out infinite 4s; }
.orb-3 { width:400px;height:400px; background:radial-gradient(circle,rgba(15,138,95,.03) 0%,transparent 70%); bottom:-160px;right:25%; animation:blob 11s ease-in-out infinite 8s; }

/* ── Navbar ──────────────────────────────────────────────── */
[data-testid="stHorizontalBlock"]:has(.nav-logo) {
    background: linear-gradient(90deg,hsl(0,0%,100%,.95),hsl(40,30%,97%,.95)) !important;
...
.nav-dot {
    ...
    background: linear-gradient(135deg, rgba(15,138,95,.2), rgba(201,162,39,.15));
...
[data-testid="stHorizontalBlock"]:has(.nav-logo) [data-testid="stColumn"]:has(.nav-scan) [data-testid="stButton"] > button:hover {
    transform: translateY(-1px) !important; box-shadow: 0 6px 24px rgba(15,138,95,.4) !important;
}
...
.glass-card:hover {
    ...
    box-shadow: 0 8px 40px rgba(15,138,95,.08), 0 0 0 1px rgba(15,138,95,.05);
...
.gradient-text {
    background: linear-gradient(135deg, hsl(153,70%,32%), hsl(43,75%,45%));
    -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
    font-weight: 800;
}
.text-shimmer {
    background: linear-gradient(90deg,hsl(153,70%,32%) 0%,hsl(43,85%,45%) 35%,hsl(153,70%,32%) 70%,hsl(43,85%,45%) 100%);
...
.icon-box {
    ...
    background: linear-gradient(135deg,rgba(15,138,95,.1),rgba(201,162,39,.05));
    border: 1px solid rgba(15,138,95,.25);
}
.gradient-border-box {
...
.gradient-border-box::before {
    ...
    background: linear-gradient(135deg,rgba(15,138,95,.4),rgba(201,162,39,.3));
...
[data-testid="stVerticalBlock"] div[data-testid="stButton"] > button:hover {
    transform: translateY(-2px) !important; box-shadow: 0 8px 28px rgba(15,138,95,.4) !important;
}
...
.stTextInput input:focus, .stNumberInput input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(15,138,95,.12) !important; outline: none !important;
}
...
[data-testid="stFileUploader"] section:hover {
    border-color: rgba(15,138,95,.4) !important; background: rgba(15,138,95,.03) !important;
}
/* Hero drop zone */
...
.home-uploader [data-testid="stFileUploader"] section:hover {
    border-color: rgba(15,138,95,.4) !important; box-shadow: 0 0 40px rgba(15,138,95,.08) !important;
}
...
[data-testid="stMetric"]:hover { transform: translateY(-2px); border-color: rgba(15,138,95,.3); }
...
::-webkit-scrollbar-thumb { background: rgba(15,138,95,.3); border-radius: 3px; }

/* ── Hero badge ─────────────────────────────────────────── */
.hero-badge {
    display: inline-flex; align-items: center; gap: 10px;
    background: linear-gradient(135deg, rgba(15,138,95,.08), rgba(201,162,39,.05));
...
.badge-dot {
    ...
    background: linear-gradient(135deg, hsl(153,70%,48%), hsl(43,75%,55%));
...
.stat-card:hover { transform: translateY(-4px); border-color: rgba(15,138,95,.3); box-shadow: 0 12px 40px rgba(15,138,95,.1); }
...
.how-card:hover { border-color:rgba(15,138,95,.25); transform:translateY(-4px); box-shadow:0 12px 40px rgba(15,138,95,.1); }
...
.scan-hist-card:hover { border-color:rgba(15,138,95,.25); transform:translateX(4px); }
...
def navbar():
    ...
    with c_scan:
        ...
        if st.button("Start Scan", key="nb_scan"):
            move_to(2)
        st.markdown('</div>', unsafe_allow_html=True)
...
def page_hero():
    ...
    with col:
        ...
        <h1 style="font-size:clamp(40px,5vw,64px); font-weight:800; line-height:1.08;
            ...
            E-Waste Value<br><span class="text-shimmer">Detector.</span>
        </h1>
        <p style="color:var(--subtle); font-size:18px; max-width:520px; margin:0 auto 48px;
                  ...
                  font-family:'DM Sans',sans-serif;">
            Snap a photo of your old electronics and instantly discover their market value,
            optimal selling channels, and environmental impact.
        </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with up_col:
            ...
            if hero_files:
                ...
                if photo_bytes_list:
                    st.markdown(f"""
                    <div style="background:rgba(15,138,95,.07);border:1px solid rgba(15,138,95,.2);
                    ...
                    if st.button("Scan Now — AI will auto-identify →", key="hero_scan_now"):
                        ...
                        with st.spinner("Identifying your device..."):
                            try:
          ...
          <div style="display:flex;justify-content:space-around;align-items:center;flex-wrap:wrap;gap:24px;">
            ...
            <div style="display:flex;align-items:center;gap:12px;">
              ...
              <div>
                ...
                <div style="font-size:12px;color:var(--subtle);">Sell or recycle</div>
              </div>
            </div>
          </div>
        </div>
        ...
        for col_obj, num, label, delay in [
            (s1, "50K+",     "Devices Scanned", "0.4s"),
            (s2, "$2.1M",    "Value Recovered",  "0.5s"),
            (s3, "1.2M lbs", "CO₂ Saved",        "0.6s"),
...
def page_how_it_works():
    ...
    with col:
        ...
        <div style='text-align:center; padding:40px 0 48px; animation: fadeInUp 0.4s ease both;'>
            <div class="step-tag">EcoLens · How It Works</div>
            <h1 style='font-size:48px; font-weight:800; margin-bottom:12px; color:#1a2e1e;
                       font-family:"Playfair Display",Georgia,serif; letter-spacing:-0.5px;'>
                Simple. Smart. <span class="text-shimmer">Sustainable.</span>
            </h1>
            <p style='color:hsl(160,8%,50%); font-size:17px; max-width:520px; margin:0 auto; line-height:1.75;'>
    ...
    steps = [
        ("📸", "Step 1", "Upload Your Device", "01",
         "Take a photo or record a short video of your device. Show the front, back, ports, and any visible damage. The more angles the better — our AI uses every pixel to assess condition.",
         "Supports JPG, PNG, WEBP photos and MP4, MOV, AVI videos.", "0.3s"),
        ("🤖", "Step 2", "AI Auto-Identification", "02",
         "Claude AI analyzes your media and automatically identifies the device name, brand, model number, manufacturing year, and screen condition — with a confidence score for each field.",
         "You only need to fill in what the AI isn't sure about.", "0.4s"),
        ("📊", "Step 3", "Live Market Pricing", "03",
         "EcoLens searches real completed eBay listings in real time to find what your exact model is actually selling for — not guesses, not outdated data. Prices reflect today's market.",
        ...
        ("📋", "Step 4", "Your Ready-to-Post Listing", "04",
         ...
         "Or choose Trade-In for store credit, or Recycle for eco-responsible disposal.", "0.6s"),
    ]
        ...
        with target_col:
            ...
            <div class="how-card" style="animation-delay:{delay};">
                <div style="display:flex; align-items:flex-start; gap:20px; margin-bottom:20px;">
                    ...
                    <div>
                        <div style="font-size:10px; color:#0F8A5F; letter-spacing:2px; font-weight:700;
                    ...
                    <div style="margin-left:auto; font-size:48px; font-weight:800;
                                color:rgba(16,185,129,0.1); line-height:1;">{num}</div>
                </div>
                ...
                <div style="background:rgba(16,185,129,0.06); border:1px solid rgba(16,185,129,0.15);
                     border-radius:8px; padding:10px 14px; font-size:12px; color:#0F8A5F;">
                    ✦ {sub}
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with mid:
        ...
        <h2 style='font-size:34px; font-weight:800; text-align:center; margin-bottom:36px;
                   color:#111827; animation: fadeInUp 0.5s ease both;'>
            Why <span class="gradient-text">EcoLens?</span>
        </h2>
        """, unsafe_allow_html=True)
        ...
        for wcol, icon, title, body in [
            ...
            (w3, "🌱", "Better for the Planet",
             "Every device that gets resold or traded in is one less in a landfill. EcoLens shows you your CO₂ impact."),
        ]:
            wcol.markdown(f"""
        ...
        # FAQ
        st.markdown("<h3 style='font-size:26px; font-weight:700; margin-bottom:8px; color:#111827;'>Frequently Asked Questions</h3>", unsafe_allow_html=True)
        st.markdown("<hr>", unsafe_allow_html=True)

        faqs = [
            ("Is EcoLens free to use?",
             "Yes, completely free. Just add your Anthropic API key in the .env file and you're ready to go."),
            ("What devices can I scan?",
             "Any consumer electronics: smartphones, laptops, tablets, monitors, cameras, gaming consoles, and more. If eBay sells it, EcoLens can price it."),
            ...
            ("Can I use this for trade-ins or recycling too?",
             "Absolutely. After the AI analysis, you can choose to get a sell listing, trade-in links (Best Buy, Apple, Amazon), or recycling center locations."),
        ]
        for q, a in faqs:
            with st.expander(q):
...
def page_dashboard():
    ...
    with col:
        ...
        <div style='padding:40px 0 32px; animation: fadeInUp 0.4s ease both;'>
            ...
            <h1 style='font-size:40px; font-weight:800; margin-bottom:8px; color:#1a2e1e;
                       font-family:"Playfair Display",Georgia,serif;'>Your Scan History</h1>
            <p style='color:hsl(160,8%,50%); margin-bottom:32px;'>Every device you've scanned this session.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not history:
            ...
            <div class="glass-card" style="text-align:center; padding:64px 40px;">
                ...
                <p style="color:#4b5563; font-size:15px; margin-bottom:28px; max-width:360px; margin-left:auto; margin-right:auto; line-height:1.7;">
                    Scan your first device and it'll show up here with the full analysis, condition grade, and estimated value.
                </p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Scan Your First Device →", key="dash_cta"):
                move_to(2)
        else:
            # Summary metrics
            total_value = sum(s.get("value", 0) for s in history)
            total_co2   = sum(s.get("co2", 0) for s in history)
            m1, m2, m3 = st.columns(3)
            m1.metric("Devices Scanned", len(history))
            m2.metric("Total Est. Value", f"${total_value:,}")
            m3.metric("CO₂ Saved (lbs)", f"{total_co2:,}")

            st.markdown("<br>", unsafe_allow_html=True)

            condition_colors = {"Excellent": "#0F8A5F", "Good": "#3aaf7a", "Fair": "#C9A227", "Poor": "#ef4444"}
            decision_icons = {"sell": "💰", "trade-in": "🏪", "recycle": "♻️"}

            for i, scan in enumerate(reversed(history)):
                cond  = scan.get("condition", "—")
                dec   = scan.get("decision", "recycle")
                color = condition_colors.get(cond, "#6b7280")
                icon  = decision_icons.get(dec, "♻️")
                delay = f"{0.1 * i:.1f}s"
                ...
                <div class="scan-hist-card" style="animation-delay:{delay};">
                    <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px;">
                        <div style="display:flex; align-items:center; gap:16px;">
                            ...
                            <div>
                                <div style="font-size:16px; font-weight:700;">{scan.get("device", "Unknown Device")}</div>
                        ...
                        <div style="display:flex; align-items:center; gap:20px;">
                            <div style="text-align:right;">
                                <div style="font-size:22px; font-weight:800; color:#0F8A5F;">
                                    ${scan.get("value", 0)}
                                </div>
                            ...
                            <div style="padding:5px 14px; border-radius:999px; font-size:11px;
                                        ...
                                        background:rgba(16,185,129,0.08); border:1px solid {color};
                                        color:{color};">
                                {cond}
                            </div>
                        </div>
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Scan Another Device →", key="dash_scan"):
                    move_to(2)
            with c2:
    ...
    with col:
        ...
        <div style='text-align:center; padding:32px 0 24px; animation:fadeInUp .4s ease both;'>
            ...
            <h2 style='font-size:clamp(24px,3vw,36px); font-weight:800; margin-bottom:8px;
                       font-family:"Playfair Display",Georgia,serif;'>Upload Your Device</h2>
            <p style='color:var(--subtle); font-size:14px; max-width:400px; margin:0 auto; line-height:1.65;'>
                Show us your device from multiple angles for the most accurate valuation.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with tab_photos:
            uploaded = st.file_uploader(
                "Upload device photos", type=["jpg","jpeg","png","webp"],
        ...
        with tab_video:
            if not CV2_AVAILABLE:
                ...
                vfile = st.file_uploader(
                    "Upload device video", type=["mp4","mov","avi","mkv"],
        ...
        if photo_bytes_list:
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,rgba(15,138,95,.08),rgba(201,162,39,.04));
                 border:1px solid rgba(15,138,95,.2); border-radius:12px;
                 padding:12px 16px; font-size:13px; color:var(--primary); margin-bottom:16px;
                 display:flex; align-items:center; gap:8px;">
                ✓ {len(photo_bytes_list)} image(s) ready · AI will auto-identify your device
            </div>
            """, unsafe_allow_html=True)
    ...
    with col:
        ...
        <div style='text-align:center; padding:32px 0 20px; animation:fadeInUp .4s ease both;'>
            ...
            <h2 style='font-size:clamp(24px,3vw,36px); font-weight:800; margin-bottom:6px;
                       font-family:"Playfair Display",Georgia,serif;'>Device Diagnostics</h2>
        </div>
        """, unsafe_allow_html=True)

        if pf:
            filled  = sum(1 for k in ("device_name","brand","model","year","powers_on","screen_condition") if not pf_unknown(k))
            unknown = 6 - filled
            if unknown > 0:
                st.markdown(f"""
                <div style='background:linear-gradient(135deg,rgba(15,138,95,.06),rgba(201,162,39,.04));
                     border:1px solid rgba(15,138,95,.2); border-radius:12px;
                ...
                st.markdown("""
                <div style='background:linear-gradient(135deg,rgba(15,138,95,.08),transparent);
                     border:1px solid rgba(15,138,95,.2); border-radius:12px;
                     padding:12px 16px; font-size:13px; color:var(--primary); margin-bottom:16px;
        ...
        with st.form("diag_form"):
            name  = st.text_input(flabel("Product Name", "device_name"), value=pf_val("device_name", ""), placeholder="e.g. iPhone 12")
            brand = st.text_input(flabel("Brand", "brand"), value=pf_val("brand", ""), placeholder="e.g. Apple")
            model = st.text_input(flabel("Model Number (optional)", "model"), value=pf_val("model", ""), placeholder="e.g. A2172")

            py = pf_val("year", 2020)
            try: py = int(py)
            except: py = 2020
            year = st.number_input(flabel("Year of Purchase", "year"), min_value=1990, max_value=2025, value=py, step=1)

            power_opts = ["Yes", "No"]
            pp = pf_val("powers_on", "Yes")
            power = st.radio(flabel("Does the device power on?", "powers_on"), power_opts,
                             index=power_opts.index(pp) if pp in power_opts else 0, horizontal=True)

            screen_opts = ["Flawless", "Minor Scratches", "Cracked", "Screen is off/broken"]
            ps = pf_val("screen_condition", "Flawless")
            screen = st.selectbox(flabel("Screen Condition", "screen_condition"), screen_opts,
                                  index=screen_opts.index(ps) if ps in screen_opts else 0)
        ...
        if submitted:
            if not name or not brand:
                st.error("Please provide at least a Product Name and Brand.")
            else:
                st.session_state.device_data = {
                    "name": name, "brand": brand,
                    "model": model or name, "year": int(year),
                    "powers_on": power, "screen": screen,
                }
                move_to(4)
...
def page_loading():
    ...
    with col:
        ...
        <div style='text-align:center; padding:60px 0 32px; animation: fadeInUp 0.4s ease both;'>
            ...
            <h2 style='font-size:32px; font-weight:800; margin-bottom:8px; color:#1a2e1e;
                       font-family:"Playfair Display",Georgia,serif;'>Analyzing Marketplace</h2>
            <p style='color:hsl(153,70%,38%); font-size:14px; font-weight:500;'>Searching live eBay sold listings · Grading condition · Building valuation</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style='background:rgba(16,185,129,0.07); border:1px solid rgba(16,185,129,0.2);
            ...
            <span style='color:#0F8A5F; font-weight:700;'>💡 Did you know?</span><br>
            {random.choice(EWASTE_FACTS)}
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.spinner(""):
            try:
                ...
                st.session_state.scan_history.append({
                    ...
                    "decision":  result.get("decision", "recycle"),
...
# ══════════════════════════════════════════════════════════════════════════════
PLACEHOLDER_IMG = "https://via.placeholder.com/600x400/0f172a/10b981?text=No+Image"
...
def page_listings_preview():
            ...
            <div style='display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px;'>
                ...
                <div style='display:flex; gap:20px; flex-shrink:0;'>
                    <div style='text-align:center;'>
                        <div style='font-size:22px; font-weight:800; color:#0F8A5F;'>${mid}</div>
    ...
    if not listings:
        ...
        with tab_area:
            ...
            for i, (tc, lst) in enumerate(zip(tab_cols, listings)):
                with tc:
                    is_sel  = (i == sel_idx)
                    bg      = "rgba(15,138,95,0.1)" if is_sel else "#F7F5EF"
                    border  = "1px solid #0F8A5F" if is_sel else "1px solid #CFE8D8"
                    label   = f"{'● ' if is_sel else ''}{lst['price_str']}"
                    st.markdown(f"""
                    <div style='background:{bg}; border:{border}; border-radius:10px;
                         padding:8px 12px; margin-bottom:4px; text-align:center;
                         animation: fadeInUp 0.3s ease both {i*0.08:.2f}s;'>
                        <div style='font-size:16px; font-weight:800;
                             color:{"#0F8A5F" if is_sel else "#111827"};'>{lst["price_str"]}</div>
        ...
        with gallery_area:
            ...
            with thumb_col:
                ...
                for j, img_url in enumerate(images[:6]):
                    is_active = (j == img_idx)
                    border_style = "2px solid #0F8A5F" if is_active else "2px solid #CFE8D8"
                    ...
                    <div style='border:{border_style}; border-radius:8px; overflow:hidden;
                         ...
                         box-shadow: {"0 0 10px rgba(16,185,129,0.3)" if is_active else "none"};'>
                    """, unsafe_allow_html=True)
                    try:
            ...
            with main_col:
                # Main image display
                st.markdown("""
                <div style='background:#F7F5EF; border:1px solid #CFE8D8;
                        ...
                        <div style='flex:1; min-width:0;'>
                            ...
                            <div style='display:flex; align-items:center; gap:8px; flex-wrap:wrap;'>
                                <span style='background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.2);
                                      color:#0F8A5F; padding:3px 10px; border-radius:999px;
                                      font-size:11px; font-weight:700;'>{sel["condition"]}</span>
                                {"<span style='background:#F7F5EF; border-radius:4px; padding:3px 8px; font-size:11px; color:#6b7280;'>" + sel["variant"] + "</span>" if sel["variant"] else ""}
                                {"<span style='color:#6b7280; font-size:12px;'>Sold " + sel["sold_date"] + "</span>" if sel["sold_date"] else ""}
                            </div>
                        </div>
                        <div style='text-align:right; flex-shrink:0;'>
                            <div style='font-size:28px; font-weight:800; color:#0F8A5F;'>{sel["price_str"]}</div>
    ...
    with nav_col:
        c1, c2 = st.columns(2)
        with c1:
            if st.button("← Back to Diagnostics", key="preview_back"):
...
def page_results():
    r        = st.session_state.analysis
    p        = st.session_state.device_data
    decision = r.get("decision", "recycle")
    ...
    with col:
        ...
        cond_colors = {"Excellent":"#0F8A5F","Good":"#3aaf7a","Fair":"#C9A227","Poor":"#ef4444"}
        ...
        <div style='padding:32px 0 16px; animation:fadeInUp .4s ease both;'>
            ...
            <span style='background:rgba(15,138,95,.08); border:1px solid {cond_color};
        ...
        badge_colors = {"sell":"var(--primary)","trade-in":"var(--accent)","recycle":"#3b82f6"}
        bc = badge_colors.get(decision, "var(--primary)")
        st.markdown(f"""
        <div class="glass-card" style="animation-delay:.15s;">
            <span style="display:inline-block; padding:4px 12px; border-radius:6px; font-size:10px;
                  ...
                  background:linear-gradient(135deg,rgba(15,138,95,.12),rgba(201,162,39,.08));
        ...
        override = st.radio(
            "Your choice:", ["Keep AI recommendation", "Sell it", "Trade it in", "Recycle it"],
            horizontal=True, label_visibility="collapsed",
        )
        st.markdown('</div>', unsafe_allow_html=True)

        decision_map = {"Keep AI recommendation": decision, "Sell it": "sell", "Trade it in": "trade-in", "Recycle it": "recycle"}
        ...
        st.markdown(f"""
        <div class="glass-card" style="text-align:center;animation-delay:.3s;">
            <p style="font-size:12px;color:var(--subtle);margin-bottom:4px;">Environmental Impact</p>
            <p class="gradient-text" style="font-size:26px;font-family:'Playfair Display',serif;margin:0;">{co2} lbs</p>
            <p style="font-size:12px;color:var(--subtle);margin-top:2px;">CO₂ kept out of landfills</p>
        </div>
        """, unsafe_allow_html=True)
...
def page_listing():
    r              = st.session_state.analysis
    p              = st.session_state.device_data
    final_decision = st.session_state.get("final_decision", r.get("decision","recycle"))
...
{listing_block.strip()}
            ...
            with c1:
                st.download_button(
                    ...
                    file_name=f"ecolens_{lst.get('title','listing')[:30].replace(' ','_')}.txt",
        ...
        # ── RECYCLE ────────────────────────────────────────────────────────────
            ...
            <div style="background:linear-gradient(135deg,rgba(15,138,95,.08),transparent);
                ...
                🌿 You're keeping <strong>{co2} lbs of CO₂</strong> out of landfills.
            </div>
            """, unsafe_allow_html=True)
            for href, icon, name, desc in [
                ("https://www.bestbuy.com/recycling", "🏪", "Best Buy Recycling", "Free drop-off electronics recycling at any location."),
                ("https://www.staples.com/sbd/cre/marketing/sustainability-center/recycling-services/", "📮", "Staples Recycling", "Free tech recycling at Staples stores nationwide."),
                ("https://earth911.com", "🌍", "Earth911", "Find a local recycling center near you."),
            ]:
                st.markdown(f"""
                <a href="{href}" target="_blank" class="glass-card" style="display:flex;align-items:center;gap:16px;text-decoration:none;margin-bottom:12px;">
                    <span style="font-size:28px;">{icon}</span>
        ...
        <div class="glass-card" style="text-align:center;margin-top:8px;">
            <p style="font-size:12px;color:var(--subtle);margin-bottom:4px;">Environmental Impact</p>
            <p class="gradient-text" style="font-size:28px;font-family:'Playfair Display',serif;margin:0;">{co2} lbs</p>
            <p style="font-size:12px;color:var(--subtle);margin-top:2px;">CO₂ kept out of landfills</p>
        </div>
        """, unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("↺ Scan Another Device →", key="restart"):
